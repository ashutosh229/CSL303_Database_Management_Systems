...
def get_conn_params():
    print(
        "Enter PostgreSQL connection details. Press Enter to accept default shown in [brackets]."
    )
    ...
    password = (
        getpass.getpass(f"Password [{DEFAULTS['password']}]: ") or DEFAULTS["password"]
    )
...
def ensure_database(params):
    try:
        ...
        if conn is None:
            print(
                "Cannot connect to postgres DB to ensure database exists. Skipping auto-create."
            )
            return False
        ...
        if not exists:
            ...
            cur.execute(
                sql.SQL("CREATE DATABASE {}").format(sql.Identifier(params["dbname"]))
            )
...
def insert_student(conn, table_name):
    try:
        cur = conn.cursor()
        print(
            "Enter student details. To stop inserting, leave name blank and press Enter."
        )
        while True:
            ...
            cur.execute(
                sql.SQL(
                    "INSERT INTO {} (name, age, department, email) VALUES (%s, %s, %s, %s) RETURNING id;"
...
def update_department_by_name(conn, table_name):
    try:
        ...
        cur.execute(
            sql.SQL(
                "UPDATE {} SET department = %s WHERE name = %s RETURNING id, name, department;"
            ).format(sql.Identifier(table_name)),
            (new_dept, name),
        )
        rows = cur.fetchall()
...
def delete_student_by_id(conn, table_name):
    try:
        ...
        try:
            ...
            return
        cur.execute(
            sql.SQL("DELETE FROM {} WHERE id = %s RETURNING id, name;").format(
...
def display_all(conn, table_name):
    try:
        cur = conn.cursor()
        cur.execute(
            sql.SQL("SELECT * FROM {} ORDER BY id;").format(sql.Identifier(table_name))
        )
        rows = cur.fetchall()
        if rows:
...
def display_by_department(conn, table_name):
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            sql.SQL("SELECT * FROM {} WHERE department = %s ORDER BY id;").format(
                sql.Identifier(table_name)
            ),
            (dept,),
        )
...
def display_avg_age_by_dept(conn, table_name):
    try:
        ...
        cur.execute(
            sql.SQL(
                "SELECT department, AVG(age)::numeric(5,2) as avg_age, COUNT(*) FROM {} GROUP BY department ORDER BY department;"
            ).format(sql.Identifier(table_name))
        )
        rows = cur.fetchall()
        if rows:
...
def display_name_startswith(conn, table_name):
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            sql.SQL("SELECT * FROM {} WHERE name ILIKE %s ORDER BY id;").format(
                sql.Identifier(table_name)
            ),
            (pattern,),
        )
...
def main():
    ...
    try:
        ...
        while True:
            ...
            if choice == "1":
                print(
                    "Creating table. You can accept default 'students' table or provide custom columns."
                )
                use_default = (
                    input(
                        "Use default students table (id, name, age, department, email)? [Y/n]: "
                    )
                    .strip()
                    .lower()
                    or "y"
                ...
                if use_default.startswith("y"):
                    ...
                    cols = []
                    print(
                        "Enter columns one per line in format: name TYPE [constraints]. Example: id SERIAL PRIMARY KEY"
                    )
